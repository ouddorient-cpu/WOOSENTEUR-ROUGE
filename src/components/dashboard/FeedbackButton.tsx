    ...
    <>
      ...
      {status === 'idle' && (
        <button
          ...
          className="fixed bottom-6 right-6 z-50 flex items-center gap-2 bg-primary text-white px-4 py-2.5 rounded-full shadow-lg shadow-primary/30 text-sm font-semibold hover:bg-primary/90 transition-all hover:scale-105"
      ...
      {(status === 'open' || status === 'sending' || status === 'sent') && (
        <div className="fixed bottom-6 right-6 z-50 w-80 bg-card border border-border rounded-2xl shadow-2xl shadow-black/10 overflow-hidden">
          {/* Header */}
