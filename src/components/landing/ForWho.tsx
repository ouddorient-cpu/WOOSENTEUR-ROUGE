    ...
    <section id="pour-qui" className="py-24 lg:py-32 bg-background">
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {t.forwho.personas.map((p, i) => {
            const meta = personaMeta[i];
            return (
              <motion.div
                key={i}
                ...
                className={`bg-white dark:bg-card rounded-2xl border-t-4 ${meta.accentBorder} border border-border hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 overflow-hidden shadow-md`}
              >
                ...
                <div className="p-6">
                  <h3 className="font-headline text-xl font-bold text-foreground mb-1">{p.title}</h3>
                  <p className="text-xs font-semibold text-muted-foreground mb-3">{p.subtitle}</p>
                  <p className="text-sm text-muted-foreground leading-relaxed mb-5">{p.description}</p>
                  <div className={`inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full border ${meta.bgAccent} ${meta.accentColor}`}>
                    ✓ {p.benefit}
                  </div>
                </div>
              </motion.div>
